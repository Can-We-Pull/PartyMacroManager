...
export class IconDropdownPanel {
  ...
  public create(parent: Frame, anchorFrame: AnchorElement): AnchorElement {
    ...
    infoText.SetText(
      `|cff888888Tip: Type to search ${this.allIcons.length} icons, or click Browse to see all|r`
    );
  ...
  private updateDropdown(): void {
    ...
    if (this.filteredIcons.length > 0 && this.filteredIcons.length < this.allIcons.length) {
      print(`|cff00ff00[PartyMacroManager]|r Found ${this.filteredIcons.length} matching icons`);
    }
  }

  private showIconPicker(anchorFrame: Button): void {
    const db = this.addon.getDB();
    ...
    const subtitle = callMethod(picker, "CreateFontString", undefined, "OVERLAY", "GameFontHighlightSmall") as FontString;
    ...
    for (let i = 0; i < iconsToShow.length; i++) {
      ...
      callMethod(btn, "SetScript", "OnClick", () => {
        ...
        callMethod(picker, "Hide");
        print(`|cff00ff00[PartyMacroManager]|r Icon set to: ${iconData.name}`);
      });
    }
